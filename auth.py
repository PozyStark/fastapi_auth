...
# # from services import get_token_id, get_token_id_sync, get_user_by_id, get_user_by_id_sync, get_user_groups, get_user_permissions, get_user_roles
...
#     permission_classes: list[BasePermission | str]
#     auth_permissions: 
...
#         auth_permissions: AuthPermissions = AuthPermissions(),
#     ):
...
#         token_in_base = await get_token_id(async_session, token_id)
...
#     async def __get_user_permissions(self, token: str) -> list[BaseModel]:
#         if not token or not self.auth_permissions.required_permissions:
#             return list()
#         user_id = jwt_token_decode(token).get('user_id')
#         user_permissions = await get_user_permissions(async_session, user_id)
#         return user_permissions
    

#     async def __get_user_roles(self, token: str) -> list[BaseModel] | None:
#         if not token or not self.auth_permissions.required_roles:
#             return list()
#         user_id = jwt_token_decode(token).get('user_id')
#         user_roles = await get_user_roles(async_session, user_id)
#         return user_roles
    

#     async def __get_user_groups(self, token: str) -> list[BaseModel] | None:
#         if not token or not self.auth_permissions.required_groups:
#             return list()
#         user_id = jwt_token_decode(token).get('user_id')
#         user_groups = await get_user_groups(async_session, user_id)
#         return user_groups
...
#     async def __get_auth_request(self, token, request) -> AuthRequest:
