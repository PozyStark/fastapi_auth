...
from schemas import AddUserRoleSchema, UpdateUserRoleSchema, UserRoleSchema
...
class UserRoleService:
    ...
    async def find_user_role_by_id(self, user_id: str):
        result = await self.repository.find_user_role_by_name(user_id)
        return result
    
    async def find_user_role_by_name(self, username: str):
        result = await self.repository.find_user_role_by_name(username)
        return result

    async def add_one(self, item: AddUserRoleSchema):
        return await self.repository.add_one(item.model_dump())
    
    async def add_many(self, items: list[AddUserRoleSchema]):
    ...
    async def update(self, item_id: int, item: UpdateUserRoleSchema):
        result = await self.repository.update_one(item.model_dump(), [self.repository.model.id == item_id])
        return result
    
    async def update_all(self, items: list[UserRoleSchema]):
